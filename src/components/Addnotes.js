...
const Addnotes = () => {
  ...
  const [notes, setNotes] = useState({ title: "", description: "", tag: "", date: "" });
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    try {
      // Add the current date when creating the note
      const currentDate = new Date().toLocaleString();

      // Attempt to add the note
      await addNote(notes.title, notes.description, notes.tag, currentDate);
      ...
      setNotes({ title: "", description: "", tag: "", date: "" });
    } catch (error) {
    ...
    <>
      ...
      <div className="container my-5">
        <div className="shadow p-4 rounded bg-light">
          <h2 className="text-center text-primary mb-4" style={{ fontFamily: 'Roboto, sans-serif' }}>
            Create New Note
          </h2>
